...
y_pred_test_result = lassoModel.predict(x_test)
# 调用均方误差函数，传入y的真实值和预测值
test_loss2 = mean_squared_error(y_test, y_pred_test_result)

# 6. 训练数据预测结果，用来划线
pred_train_result= lassoModel.predict( polynomial(X, 20))
# 画出拟合曲线，并标出误差
ax[0,1].plot(X, pred_train_result , 'r')
ax[0,1].text(-3, 1, f"测试集均方误差：{test_loss2:.4f}")
ax[0,1].text(-3, 1.3, "Lasso回归")
# 画出所有系数的柱状图
ax[1,1].bar(np.arange(20), lassoModel.coef_.reshape(-1))

# print(lassoModel.coef_)

for idx, item in enumerate(lassoModel.coef_):
    print(f"{idx}: {item:.5f}")
